...
[condition][]with amount over {amount}=eval($amount>={amount})
[condition][]that originates from {ctry_list}=eval("{ctry_list}".contains($ctry.name()))
[consequence][]Set risk check eligibility to {eligibility}=$o.setRiskStatus("{eligibility}"); System.out.println("Setting RiskCheck = true for order"+$o);
